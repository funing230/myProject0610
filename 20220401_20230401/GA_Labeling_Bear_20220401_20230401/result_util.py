...
def get_pairstrategy_return():
    BTC = yf.download('BTC-USD',start=datetime(2018, 1, 1), end=datetime(2023, 5, 14))
    ETH = yf.download('ETH-USD', start=datetime(2018, 1, 1), end=datetime(2023, 5, 14))
    ...
    split = 271  # int(len(pair_ret) * 0.7)
    ...
    up_lw.dropna()
    rbtc_ret = pair_ret['BTC_RET'].tail(1686)   # .tail(int(len(pair_ret) * 0.3))
    reth_ret = pair_ret['ETH_RET'].tail(1686)   # .tail(int(len(pair_ret) * 0.3))
    # return on returns
