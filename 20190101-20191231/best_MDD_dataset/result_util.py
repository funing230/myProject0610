...
def get_pairstrategy_return():
    BTC = yf.download('BTC-USD',start=datetime(2018, 10, 1), end=datetime(2019, 12, 31))
    ETH = yf.download('ETH-USD', start=datetime(2018, 10, 1), end=datetime(2019, 12, 31))
    ...
    up_lw.dropna()
    rbtc_ret = pair_ret['BTC_RET'].tail(365)   # .tail(int(len(pair_ret) * 0.3))
    reth_ret = pair_ret['ETH_RET'].tail(365)   # .tail(int(len(pair_ret) * 0.3))
    # return on returns
